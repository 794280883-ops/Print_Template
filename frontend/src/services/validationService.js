import { BARCODE_FORMATS, FIELD_DICT, SUPPORTED_TYPES } from "../data/constants.js";

const SUPPORTED_BARCODE_FORMATS = new Set(BARCODE_FORMATS.map((format) => format.value));

export function validateTemplateDsl(templateDsl, fields = null) {
  const errors = [];
  const warnings = [];
  const tips = [];
  const template = templateDsl || {};
  const fieldList = fields || FIELD_DICT[template.templateType] || [];
